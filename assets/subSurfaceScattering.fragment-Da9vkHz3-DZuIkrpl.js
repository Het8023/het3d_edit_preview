import{I as e,l as c,af as f}from"./index-DdEn_fc_.js";const i="fibonacci",o=`#define rcp(x) 1./x
#define GOLDEN_RATIO 1.618033988749895
vec2 Golden2dSeq(int i,float n)
{return vec2(float(i)/n+(0.5/n),fract(float(i)*rcp(GOLDEN_RATIO)));}
vec2 SampleDiskGolden(int i,int sampleCount)
{vec2 f=Golden2dSeq(i,float(sampleCount));return vec2(sqrt(f.x),TWO_PI*f.y);}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=o);const s={name:i,shader:o},t="diffusionProfile",n="uniform vec3 diffusionS[5];uniform float diffusionD[5];uniform float filterRadii[5];";e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=n);const S={name:t,shader:n},a="subSurfaceScatteringPixelShader",l=`#include<helperFunctions>
#include<fibonacci>
...
totalWeight=max(totalWeight,HALF_MIN);gl_FragColor=vec4(inputColor.rgb+albedo*max(totalIrradiance/totalWeight,vec3(0.0)),1.);}`;e.ShadersStore[a]||(e.ShadersStore[a]=l);const d=[c,s,f,S];for(const r of d)e.IncludesShadersStore[r.name]||(e.IncludesShadersStore[r.name]=r.shader);const p={name:a,shader:l};export{p as subSurfaceScatteringPixelShader};
